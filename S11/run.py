...
def run_model_test(model, train_loader, test_loader, epochs, device, learning_rate, **regularization):
    ...
    # test_lrs =[round(x,3) for x in np.linspace(3.68E-02/10,3.68E-02,epochs)]
    test_lrs =[round(x,3) for x in np.linspace(0.001,0.1,epochs)]
    for epoch in range(1, epochs + 1):
        
        learning_rate = test_lrs[epoch-1]
        optimizer = optim.SGD(model.parameters(), lr=learning_rate, momentum=0.9, weight_decay=l2_factor)
        print(f'\nEpoch {epoch}:')
        train(model,train_loader, criterion, optimizer, device,l1_factor =l1_factor, **train_trackers)
        # scheduler.step()
    ...
    return model,train_trackers,test_trackers,incorrect_samples,test_lrs